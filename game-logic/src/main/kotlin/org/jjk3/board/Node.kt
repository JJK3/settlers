...
data class NodeNumber(val n: Int) {
    ...
    /** Going counter-clockwise, get the coordinates of the previous Edge. */
    fun prevEdge() = EdgeNumber(n)

    /** Going clockwise, get the coordinates of the next Edge. */
...
class Node {
    ...
    fun allCoords() = hexes.map { NodeCoordinate(it.key.coords, it.value) }
    fun isAt(coord: NodeCoordinate) = allCoords().contains(coord)
    ...
    override fun toString(): String = "Node(city=$city, port=$port, coords=${coords()})"
