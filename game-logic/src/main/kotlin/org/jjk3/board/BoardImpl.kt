...
object TileBag {
    fun newBag(randAdd: Int = 0): Bag<Hex> {
        ...
        (1..randAdd).forEach {
            val randomResource = Resource.values().toList().pickRandom()
...
/** The Standard Board */
open class StandardBoard : Board() {
    init {
        ...
        val coords2 = listOf(EdgeCoordinate(1, 0,
                ...
                EdgeCoordinate(- 1, 3, 4), EdgeCoordinate(- 2, 2, 4), EdgeCoordinate(- 2, 1, 5),
                EdgeCoordinate(- 1, 0, 0))
...
/**
 * A square board where x and y dimensions are the same.
 */
open class SquareBoard(side: Int) : Board() {
    init {
        var bag: Bag<Hex> = TileBag.newBag(side * side)
        //Tiles
        var coords: List<HexCoordinate> = emptyList()
        for (x in 1..side) {
            for (y in 1..side) {
                coords += HexCoordinate(x, y)
            }
        }
